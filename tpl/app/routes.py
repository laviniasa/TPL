...
@routes.route("/checkin-carrinho/<int:reserva_id>", methods=["POST"])
def checkin_carrinho(reserva_id):
    ...
    # =====================================================
    # HORÁRIO PROGRAMADO DA RESERVA
    # =====================================================

    inicio_programado = datetime.combine(
        reserva.data_inicio,
        reserva.hora_inicio
    )

    fim_programado = datetime.combine(
        reserva.data_inicio,
        reserva.hora_fim
    )

    # =====================================================
    # NÃO PERMITIR CHECK-IN ANTES DO HORÁRIO
    # =====================================================

    if agora < inicio_programado:
        return redirect(
            url_for(
                "routes.agenda",
    ...
    # =====================================================
    # VERIFICAR SE JÁ EXISTE UM USO ATIVO
    # =====================================================

    uso = (
        UsoCarrinho.query
        .filter_by(reserva_id=reserva.id)
        .order_by(UsoCarrinho.id.desc())
        .first()
    )

    # Se já está com o carrinho, não cria outro check-in
    if uso and uso.checkout_em is None:
        return redirect(
            url_for(
                "routes.agenda",
                data=reserva.data_inicio.isoformat()
...
@routes.route("/checkout-carrinho/<int:reserva_id>", methods=["POST"])
def checkout_carrinho(reserva_id):
    ...
    # Evita fazer checkout duas vezes
    if uso.checkout_em is not None:
        return redirect(url_for("routes.agenda"))
